        ...
        <div className="fixed top-0 left-0 right-0 z-50 p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full flex justify-center items-center mx-1">
            <div className="mt-14 relative p-4 w-full max-w-xs max-h-full">
                <div className="relative bg-black rounded-lg shadow">
                    <Button icon={<Icon type="close" iconClass="w-3 h-3" />} onClick={() => { closeModal() }} buttonClass="absolute top-3 end-2.5 text-sm w-8 h-8 ms-auto inline-flex justify-center items-center font-medium" isTextVisible={false} text={"close"} />
                    <div className="p-4 md:p-5 text-center">
                        ...
                        <Button text="Close" buttonClass="p-2 text-md" isTextVisible={true} onClick={() => { closeModal() }} />
                    </div>
                </div>
            </div>
        </div>
