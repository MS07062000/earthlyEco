...
interface SelectAddressModalProps {
  listOfAddressInfo: address[];
  selectAddress: address | null;
  setSelectAddress: React.Dispatch<React.SetStateAction<address | null>>;
  setShowSelectAddressModal: React.Dispatch<React.SetStateAction<boolean>>;
              ...
              {listOfAddressInfo.map((addressInfo: address, index: number) => (
                <li key={index}>
                  <input
                    ...
                    name="address"
                    value={index}
                    checked={selectAddress === addressInfo}
                    onChange={() => setSelectAddress(addressInfo)}
                    className="accent-[#FDD35B]"
                  />
                  <AddressCard
