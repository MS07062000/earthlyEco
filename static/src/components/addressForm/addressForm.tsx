...
import { useAppSelector } from "../../store/hooks/apphook";
...
const AddressForm: React.FC<AddressFormProps> = ({
  ...
  const { addAddress, editAddress } = useAddressHook();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const { landmark, addressLine2, ...addressCardfieldsRequired } =
      addressCard;

    const isEmpty = Object.values(addressCardfieldsRequired).every(
      (value) => value === ""
    );
  ...
  return (
    <div className="mt-16 mx-auto max-w-md space-y-6 min-h-screen pb-8">
      <div className="space-y-2 text-center">
        <h1 className="text-3xl font-bold">
          {isAdd ? "Add New" : "Edit"} Address
        </h1>
      </div>
      <form onSubmit={handleSubmit}>
        <div
          className="rounded-lg md:shadow-[-10px_10px_20px_#d7b34d,10px_-10px_20px_#fff369] bg-card text-card-foreground"
          data-v0-t="card"
        >
          <div className="p-2 md:p-6 space-y-4">
            ...
            ].map(({ id, label, value, onChange, required }) => (
              <Input
                id={id}
                label={label}
                value={value}
            ...
            <Button
              id="submitAddress"
              type="submit"
              text={isAdd ? "Add Address" : "Save Address"}
              isTextVisible={true}
              buttonClass="w-full text-sm px-5 py-2.5"
            />
            {successMessage != null && (
              <MessageModal
                isSuccess={true}
